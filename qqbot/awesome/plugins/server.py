...
from Translate.TranslateModule import *
...
@on_command('服务器', only_to_me=False, aliases=['', ])
...
@on_command('接收', only_to_me=False, aliases=['', ], permission=perm.GROUP_ADMIN)
async def _(session: CommandSession):
...
@on_command('发送', only_to_me=False, aliases=['', ], permission=perm.GROUP_ADMIN)
async def _(session: CommandSession):
...
@on_command('艾特发送', only_to_me=False, aliases=['@发送', ], permission=perm.GROUP_ADMIN)
async def _(session: CommandSession):
...
@on_command('世界状态', only_to_me=False, aliases=["世界信息", ])
async def _(session: CommandSession):
    ctx = session.ctx.copy()
    if "group_id" not in ctx.keys():
        return
    ...
    try:
        ...
        Gameseason = worldstatedict["season"]
        worldstate = worldstate + "季节:{}\n".format(GetSeasonString(Gameseason))
        worldstate = worldstate + \
            "温度:{}\n".format(int(worldstatedict["temperature"]))
        worldstate = worldstate + \
...
@on_command('玩家列表', only_to_me=False, aliases=['玩家信息', ])
async def _(session: CommandSession):
    ctx = session.ctx.copy()
    if "group_id" not in ctx.keys():
        return
    ...
    try:
        ...
        for playersdict in playerslist:
            prefabName = str(playersdict["prefab"])
            playersinfo = playersinfo + \
                "{}【{}】\n".format(
                    playersdict["name"], GetPlayerPrefabName(prefabName))
    except Exception as e:
        logger.error(e)
    if playersinfo == "":
